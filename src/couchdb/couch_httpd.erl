...
parse_post(Req) ->
    try do_parse(Req) of Str -> ?l2b(Str)
    catch _:_ -> "" end.

do_parse(#httpd{method='POST'} = Req) ->
    {Props} = couch_httpd:json_body_obj(Req),
    Raw = couch_util:get_value(<<"views">>, Props),
    [Spec] = couch_index_merger_validation:views_param(Raw),
    #set_view_spec{name = Bucket, ddoc_id = DDoc, view_name = View} = Spec,
